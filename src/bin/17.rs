...
pub fn part_two(input: &str) -> Option<u64> {
    ...
    for i in 0..total {
        ...
        if let Some(last_index) = seen.insert((jet_index, rock_index, snap), i) {
            ...
            let n = (total - i) / cycle;
            let m = (total - i) % cycle;
            println!("rock_index: {}, jet_index: {}, cycle: {:?}, m: {}, n: {}", rock_index, jet_index, cycle, m, n);
            println!("last_index: {}, last_height: {}", last_index, heights[last_index]);
            return Some(heights[m + last_index - 1] as u64 + (n+1) as u64 * (chamber.cur_height - heights[last_index]) as u64);
        }
...
impl Chamber {
    ...
    fn can_move(&self, rock: &Rock, start_pos: (i64, i64), diff: (i64, i64)) -> bool {
        rock.blocks.iter().all(|(x, y)| {
            ...
            new_y >= 0 && new_y < 7 && new_x >= 0 && self.grid[new_x as usize][new_y as usize] != '#' 
        })
    }
